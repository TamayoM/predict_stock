...
st.title("AIで株価予測アプリ")
st.write("AIを使って株価を予測してみましょう")

image = Image.open('stock_predict.png')
st.image(image, use_column_width=True)

st.write("※あくまでも予測です（参考値）。こちらのアプリによる損害や損失は一切補償しかねます。")
st.header("株価銘柄のティッカーシンボルを入力してください。")
stock_name = st.text_input("例: AAPL, FBM SFTBY（大文字、小文字どちらでも可)","APPL")
...
def stock_predict():
    ...
    if accuracy > 0.75:
        ...
        st.write('信頼度:低')
    st.write('オレンジの線（Predict）は予測値です')
